...
## IMPORTANT: Supported Conditional Syntax

**Only these conditional syntaxes are supported:**
- `[if placeholder]` - checks if placeholder has a value (not empty)
- `[if placeholder=value]` - checks if placeholder equals value exactly
- `[if placeholder][else][/if]` - if-else block with either syntax above

**Comparison operators (`<`, `>`, `<=`, `>=`, `!=`) are NOT supported** and will cause validation errors.

To check ranges, use multiple equality checks instead:
```bbcode
[if pictureCount=1]Single image[/if]
[if pictureCount=50]Medium gallery[/if]
...
**Note:** Can generate very long output for galleries with many images. Use conditionally with equality check:
```bbcode
[if pictureCount=1]
#allImages#
[else]
[i]Multiple images (see download link)[/i]
[/if]
```
...
**Example 3: Metadata indicator**
```bbcode
[if custom3]
  [b]Location:[/b] #custom3#
[else]
  [b]Location:[/b] [i]Not specified[/i]
[/if]
```
...
#allImages#
[else]
[b]Preview:[/b] [i]Multiple images - see download[/i]
[/if]
...
### Multiple Gallery Size Categories (Using Equality Checks)

**Example:**
```bbcode
[if pictureCount=1]
  [color=green][b]Single image[/b][/color]
[/if]
...
**Note:** Only equality (`=`) is supported. To create range-based conditions, use multiple equality checks for specific values. Comparison operators (`<`, `>`, `<=`, `>=`) are NOT supported.
