...
Usage:
    python multi_host_uploader.py <host> <file> [api_key]

Examples:
    python multi_host_uploader.py gofile test.jpg
    python multi_host_uploader.py pixeldrain test.jpg your_api_key_here
    python multi_host_uploader.py litterbox test.jpg 24h
    python multi_host_uploader.py filedot test.jpg username:password
    python multi_host_uploader.py rapidgator test.jpg username:password

For imxup integration:
    Upload gallery folder: python "path/to/multi_host_uploader.py" gofile "%p"
    Upload as ZIP:         python "path/to/multi_host_uploader.py" gofile "%z"
...
class MultiHostUploader:
    ...
    def _login_session_based(self, credentials: str) -> None:
        ...
        try:
            response = self.session.post(self.config.login_url, data=login_data, timeout=30)
